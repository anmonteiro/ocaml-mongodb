...
let attr_int_encoding attrs =
  match Ppx_deriving.attr ~deriver "encoding" attrs |>
        Ppx_deriving.Arg.(get_attr ~deriver (enum ["string"; "int32"; "int64"])) with
  | Some "string" -> `String
  | Some "int32" -> `Int32
  | Some "int64" | None -> `Int64
  | _ -> assert false
...
let attr_ser attrs =
  Ppx_deriving.(attrs |> attr ~deriver "to_bson" |> Arg.(get_attr ~deriver expr))
let attr_desu attrs =
  Ppx_deriving.(attrs |> attr ~deriver "of_bson" |> Arg.(get_attr ~deriver expr))
...
and ser_expr_of_only_typ typ =
  ...
  let attr_int_encoding typ =
    match attr_int_encoding typ with `String -> "String" | `Int32 -> "Int32" | `Int64 -> "Int64"
  in
  match typ with
  | [%type: unit]            -> [%expr fun (x:Ppx_deriving_runtime.unit) -> `Null]
  | [%type: int]             -> [%expr fun (x:Ppx_deriving_runtime.int) -> `Int64 (Int64.of_int x)]
  | [%type: float]           -> [%expr fun (x:Ppx_deriving_runtime.float) -> `Double x]
  | [%type: bool]            -> [%expr fun (x:Ppx_deriving_runtime.bool) -> `Boolean x]
  | [%type: string]          -> [%expr fun (x:Ppx_deriving_runtime.string) -> `String x]
  | [%type: bytes]           -> [%expr fun x -> `Binary (Generic (Bytes.to_string x))]
  | [%type: char]            -> [%expr fun x -> `String (String.make 1 x)]
  | [%type: [%t? typ] ref]   -> [%expr fun x -> [%e ser_expr_of_typ typ] !x]
  | [%type: [%t? typ] list]  -> [%expr fun x -> `Array (safe_map [%e ser_expr_of_typ typ] x)]
  | [%type: int32] | [%type: Int32.t] -> [%expr fun x -> `Int32 x]
  ...
  | [%type: [%t? typ] array] ->
    [%expr fun x -> `Array (Array.to_list (Array.map [%e ser_expr_of_typ typ] x))]
  | [%type: [%t? typ] option] ->
    [%expr function None -> `Null | Some x -> [%e ser_expr_of_typ typ] x]
  | [%type: Bson.t] -> [%expr fun x -> x]
  | { ptyp_desc = Ptyp_constr ({ txt = lid; _ }, args); _ } ->
    let fwd = app (Exp.ident (mknoloc (Ppx_deriving.mangle_lid (`Suffix "to_bson") lid)))
  ...
  | { ptyp_desc = Ptyp_variant (fields, _, _); ptyp_loc; _ } ->
      ...
      fields |> List.map (fun (field: row_field) ->
        ...
        | Rtag(label, false, [{ ptyp_desc = Ptyp_tuple typs; _ }]) ->
          ...
          Exp.case (Pat.variant label (Some (ptuple (List.mapi (fun i _ -> pvar (argn i)) typs))))
                   [%expr `Array ((`String [%e str (attr_name label attrs)]) :: [%e
                      list (List.mapi
    ...
    in
    Exp.function_ cases
  | { ptyp_desc = Ptyp_var name; _ } -> [%expr ([%e evar ("poly_"^name)] : _ -> Bson.t)]
...
and desu_expr_of_only_typ ~path typ =
  ...
  | [%type: int]    -> decode [%pat? `Int64 x]    [%expr Result.Ok x]
  | [%type: float]  ->
    decode' [[%pat? `Int32 x],    [%expr Result.Ok (Int32.to_float x)];
  ...
  | [%type: bytes]  -> decode [%pat? `Binary (Generic x)] [%expr Result.Ok (Bytes.of_string x)]
  | [%type: char]   ->
  ...
  | [%type: nativeint] | [%type: Nativeint.t] ->
    ...
    | _ ->
      decode' [[%pat? `Int64 x],    [%expr Result.Ok (Int64.to_nativeint x)];
  ...
  | [%type: Bson.t] -> [%expr fun x -> Result.Ok x]
  | { ptyp_desc = Ptyp_tuple typs; _ } ->
    decode [%pat? `Array [%p plist (List.mapi (fun i _ -> pvar (argn i)) typs)]]
  ...
  | { ptyp_desc = Ptyp_variant (fields, _, _); ptyp_loc; _ } ->
    ...
    let tag_cases = tags |> List.map (fun field ->
      ...
      | Rtag(label, true (*empty*), []) ->
        let label = label.txt in
        let attrs = field.prf_attributes in
        Exp.case [%pat? `Array [`String [%p pstr (attr_name label attrs)]]]
                 [%expr Result.Ok [%e Exp.variant label None]]
      | Rtag(label, false, [{ ptyp_desc = Ptyp_tuple typs; _ }]) ->
        let label = label.txt in
        let attrs = field.prf_attributes in
        Exp.case [%pat? `Array ((`String [%p pstr (attr_name label attrs)]) :: [%p
                    plist (List.mapi (fun i _ -> pvar (argn i)) typs)])]
                 (desu_fold ~loc ~path (fun x -> (Exp.variant label (Some (tuple x)))) typs)
      | Rtag(label, false, [typ]) ->
        let label = label.txt in
        let attrs = field.prf_attributes in
        Exp.case [%pat? `Array [`String [%p pstr (attr_name label attrs)]; x]]
    ...
    and inherits_case =
      ...
      |> Exp.case [%pat? _]
    in
    [%expr fun (json : Bson.t) ->
      [%e Exp.match_ [%expr json] (tag_cases @ [inherits_case])]]
  | { ptyp_desc = Ptyp_constr ({ txt = lid; _ }, args); _ } ->
     let fwd = app (Exp.ident (mknoloc (Ppx_deriving.mangle_lid (`Suffix "of_bson") lid)))
...
let ser_type_of_decl ~options ~path:_ type_decl =
  ...
  let polymorphize = Ppx_deriving.poly_arrow_of_type_decl
                       (fun var -> [%type: [%t var] -> Bson.t]) type_decl in
  polymorphize [%type: [%t typ] -> Bson.t]

let ser_str_of_record ~loc varname labels =
  let fields =
    labels |> List.mapi (fun _i { pld_loc = loc; pld_name = { txt = name; _ }; pld_type; pld_attributes; _ } ->
...
let ser_str_of_type ~options ~path ({ ptype_loc = loc; _ } as type_decl) =
  ...
  | Ptype_open -> begin
    let to_bson_name = Ppx_deriving.mangle_type_decl (`Suffix "to_bson") type_decl in
    let mod_name = Ppx_deriving.mangle_type_decl
      (`PrefixSuffix ("M", "to_bson")) type_decl
    in
    match type_decl.ptype_manifest with
    | Some ({ ptyp_desc = Ptyp_constr ({ txt = lid; _ }, _args); _ } as manifest) ->
    ...
    | None ->
      ...
      let polymorphize_ser  = Ppx_deriving.poly_arrow_of_type_decl
        (fun var -> [%type: [%t var] -> Bson.Safe.t]) type_decl
      in
      let ty = Typ.poly poly_vars (polymorphize_ser [%type: [%t typ] -> Bson.Safe.t]) in
      let default_fun =
        ...
        [%expr fun _ ->
          invalid_arg ("to_bson: Maybe a [@@deriving bson] is missing when extending the type "^
                       [%e e_type_path])]
      in
      ...
      in
      ([mod_],
       [Vb.mk (pvar to_bson_name) [%expr fun x -> [%e field] x]],
       [])
  end
  | kind ->
    let serializer =
      ...
      | Ptype_variant constrs, _ ->
        constrs
        |> List.map (fun { pcd_name = { txt = name'; _ }; pcd_args; pcd_attributes; _ } ->
          ...
          | Pcstr_tuple(args) ->
            ...
            Exp.case
              ...
              [%expr `Array ((`String [%e str json_name]) :: [%e list arg_exprs])]
          | Pcstr_record labels ->
            let arg_expr = ser_str_of_record ~loc (argn 0) labels in
            Exp.case
              (pconstr name' [pvar(argn 0)])
              [%expr `Array ((`String [%e str json_name]) :: [%e list[arg_expr]])]
          )
        |> Exp.function_
...
let ser_str_of_type_ext ~options ~path:_ ({ ptyext_path = { loc; _ }; _} as type_ext) =
  ignore (parse_options options);
  let serializer =
    let pats =
      List.fold_right (fun { pext_name = { txt = name'; _ }; pext_kind; pext_attributes; _ } acc_cases ->
  ...
  let mod_name =
    let mod_lid =
      Ppx_deriving.mangle_lid
        (`PrefixSuffix ("M", "to_bson")) type_ext.ptyext_path.txt
    in
...
let desu_type_of_decl_poly ~options ~path:_ type_decl type_ =
  ...
  let loc = type_decl.ptype_loc in
  let polymorphize = Ppx_deriving.poly_arrow_of_type_decl
                       (fun var -> [%type: Bson.t -> [%t error_or var]]) type_decl in
  polymorphize type_

let desu_type_of_decl ~options ~path type_decl =
  let loc = type_decl.ptype_loc in
  let typ = Ppx_deriving.core_type_of_type_decl type_decl in
  desu_type_of_decl_poly ~options ~path type_decl [%type: Bson.t -> [%t error_or typ]]
...
let desu_str_of_record ~loc ~is_strict ~error ~path wrap_record labels =
  ...
  let cases =
    (labels |> List.mapi (fun i { pld_loc = loc; pld_name = { txt = name; _ }; pld_type; pld_attributes; _ } ->
...
let desu_str_of_type ~options ~path ({ ptype_loc = loc; _ } as type_decl) =
  ...
  match type_decl.ptype_kind with
  | Ptype_open -> begin
    let of_bson_name = Ppx_deriving.mangle_type_decl (`Suffix "of_bson") type_decl in
    let mod_name = Ppx_deriving.mangle_type_decl
      (`PrefixSuffix ("M", "of_bson")) type_decl
    in
    match type_decl.ptype_manifest with
    | Some ({ ptyp_desc = Ptyp_constr ({ txt = lid; _ }, _args); _ } as manifest) ->
      ...
      let orig_mod = Mod.ident (mknoloc lid) in
      let poly_desu = polymorphize [%expr ([%e wrap_runtime desu] : Bson.t -> _)] in
      ([Str.module_ (Mb.mk (mod_mknoloc mod_name) orig_mod)],
    ...
    | None ->
      ...
      in
      ([mod_],
       [Vb.mk (pvar of_bson_name) [%expr fun x -> [%e field] x]],
       [])
  end
  | kind ->
    let desurializer =
      ...
      | Ptype_variant constrs, _ ->
        let cases = List.map (fun { pcd_loc = loc; pcd_name = { txt = name'; _ }; pcd_args; pcd_attributes; _ } ->
          match pcd_args with
          | Pcstr_tuple(args) ->
            Exp.case
              [%pat? `Array ((`String [%p pstr (attr_name name' pcd_attributes)]) ::
...
let desu_str_of_type_ext ~options ~path ({ ptyext_path = { loc; _ }; _ } as type_ext) =
  ignore(parse_options options);
  let desurializer =
    let pats =
      List.fold_right (fun { pext_name = { txt = name'; _ }; pext_kind; pext_attributes; _ } acc_cases ->
        ...
        | Pext_decl (pext_args, _) ->
          let case =
            ...
            | Pcstr_tuple(args) ->
              Exp.case
                [%pat? `Array ((`String [%p pstr (attr_name name' pext_attributes)]) ::
                                       [%p plist (List.mapi (fun i _ -> pvar (argn i)) args)])]
                (desu_fold ~loc ~path (fun x -> constr name' x) args)
            | Pcstr_record _ ->
              raise_errorf ~loc "%s: record variants are not supported in extensible types" deriver
  ...
  let mod_name =
    let mod_lid =
      Ppx_deriving.mangle_lid
        (`PrefixSuffix ("M", "of_bson")) type_ext.ptyext_path.txt
    in
    Longident.name mod_lid
  in
  let polymorphize = Ppx_deriving.poly_fun_of_type_ext type_ext in
...
let ser_sig_of_type ~options ~path type_decl =
  let to_bson =
    Sig.value (Val.mk (mknoloc (Ppx_deriving.mangle_type_decl (`Suffix "to_bson") type_decl))
  ...
  | Ptype_open ->
    ...
    let loc = typ.ptyp_loc in
    let polymorphize_ser  = Ppx_deriving.poly_arrow_of_type_decl
      (fun var -> [%type: [%t var] -> Bson.t]) type_decl
    in
    let ty = Typ.poly poly_vars (polymorphize_ser [%type: [%t typ] -> Bson.t]) in
    let typ = Type.mk ~kind:(Ptype_record
       [Type.field ~mut:Mutable (mknoloc "f") ty]) (mknoloc "t_to_bson")
    in
...
let desu_sig_of_type ~options ~path type_decl =
  ...
  let of_bson =
    Sig.value (Val.mk (mknoloc (Ppx_deriving.mangle_type_decl (`Suffix "of_bson") type_decl))
                      (desu_type_of_decl ~options ~path type_decl))
  in
  let typ = Ppx_deriving.core_type_of_type_decl type_decl in
  let loc = typ.ptyp_loc in
  let of_bson_exn =
    Sig.value (Val.mk (mknoloc (Ppx_deriving.mangle_type_decl (`Suffix "of_bson_exn") type_decl))
                      (desu_type_of_decl_poly ~options ~path type_decl [%type: Bson.t -> [%t typ]]))
  in
  match type_decl.ptype_kind with
  | Ptype_open ->
    let mod_name = Ppx_deriving.mangle_type_decl
    ...
    in
    let typ = Type.mk ~kind:(Ptype_record
       [Type.field ~mut:Mutable (mknoloc "f") ty]) (mknoloc "t_of_bson")
    in
  ...
  | _ ->
    [of_bson]
    @ (if not want_exn then [] else [of_bson_exn])
...
let bson_str_fields ~options ~path:_ type_decl =
  ...
  | true, kind ->
    ...
    | Ptype_record labels, _ ->
      let loc = !Ast_helper.default_loc in
      let fields =
        labels |> List.map (fun { pld_name = { txt = name; _ }; pld_attributes; _ } ->
      ...
      in
        [
          Str.module_ (Mb.mk (mod_mknoloc (Ppx_deriving.mangle_type_decl (`Prefix "Bson_meta") type_decl))
                      (Mod.structure [
...
let bson_sig_fields ~options ~path:_ type_decl =
  ...
  | true, kind ->
    ...
    | Ptype_record _, _ ->
      let loc = !Ast_helper.default_loc in
      [
        Sig.module_ (Md.mk (mod_mknoloc (Ppx_deriving.mangle_type_decl (`Prefix "Bson_meta") type_decl))
                    (Mty.signature [
